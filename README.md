...
Claude is instructed to open a pull request, so the issue closes through
the standard PR review-and-merge flow rather than from inside the run.
...
and the subcommand exits 0. Claude is told to open a PR; the issue closes
through the standard PR review-and-merge flow.
